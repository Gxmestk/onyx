...
class HuggingFaceTokenizer(BaseTokenizer):
    def __init__(self, model_name: str):
        ...
        # corpus-patch: tokenizer.json may ship with a truncation block (e.g.
        # aisingapore/SEA-LION-E5-Embedding-600M has max_length=512). The raw
        # `tokenizers` lib honors it, silently capping encode() at 512 — which
        # makes the chunker under-count and emit oversized chunks that the
        # embedding server rejects (400). Disable it so counts are true lengths.
        if self.encoder.truncation is not None:
...
def _try_initialize_tokenizer(
    ...
    if model_provider is not None:
        ...
        # corpus-patch: tiktoken only recognizes OpenAI model names. For a
        # non-OpenAI model served via a provider (e.g. aisingapore/SEA-LION-E5
        # behind provider_type=litellm), fall back to the HuggingFace tokenizer
        # so chunking uses the model's real subword vocabulary instead of the
        # nomic default (which under-counts Thai ~2.7x and overflows the model).
        try:
            tokenizer = HuggingFaceTokenizer(model_name)
            logger.info(
                "Initialized HuggingFaceTokenizer (provider fallback) for: %s",
                model_name,
            )
            return tokenizer
